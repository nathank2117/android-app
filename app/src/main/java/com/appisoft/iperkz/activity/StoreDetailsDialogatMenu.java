...
public class StoreDetailsDialogatMenu extends DialogFragment {
    Store store;
    TableLayout tl;
    ...
    public StoreDetailsDialogatMenu(Store store) {
        super();
        this.store = store;
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
      ...
      /*  if((store.getStoreTypeId()==4) && (store.getStoreStatus().equalsIgnoreCase("OPEN"))){
        ...
        getDrivingDistance(store);

        loginViewModel.getDrivingDistanceResult( ).observe(this, new Observer<String>() {
            @Override
...
//        if((store.getStoreTypeId()==4) && (store.getStoreStatus().equalsIgnoreCase("OPEN"))){
  ...
  //      }
    ...
    private void getDrivingDistance(Store store) {
        ...
        loginViewModel.getDrivingDistanceResultMenu(store, userLocation, this.getContext());
    }
