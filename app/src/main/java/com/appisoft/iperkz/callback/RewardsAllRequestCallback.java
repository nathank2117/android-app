...
public class RewardsAllRequestCallback extends UrlRequest.Callback {
    ...
    private MutableLiveData<Reward[]>  rewardsList;
    public RewardsAllRequestCallback(Context context, MutableLiveData<Reward[]> rewardsList ) {
                                       ...
                                       public void run() {
                                           if ((returnedData != null ) && (rewards.size()>0)) {
                                               ...
                                               for (LinkedHashMap<String, Object> map : rewards) {
                                                   ...
                                                   data.add(receivedReward);

                                               }
                                               Reward[] myList = new Reward[data.size()];
                                               data.toArray(myList);
                                               rewardsList.setValue(myList);

                                           } else{
                                               rewardsList.setValue(null);
                                           }
                                       }
                                   }
