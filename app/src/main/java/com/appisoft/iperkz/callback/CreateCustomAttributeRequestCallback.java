...
public class CreateCustomAttributeRequestCallback extends UrlRequest.Callback {

    private static final String TAG = "CreateCustomAttsRequest";
    ...
    private MutableLiveData<AddressDetail> createCustomAttributeResult;

    public CreateCustomAttributeRequestCallback(Context context, MutableLiveData<AddressDetail> createCustomerResult ) {

            this.mainActivity = (PaymentActivity)context;

        this.createCustomAttributeResult = createCustomerResult;
    }
    ...
    public void onSucceeded(UrlRequest request, UrlResponseInfo info) {
        ...
        final List<LinkedHashMap<String, Object>> addressDetailsItems =  (List<LinkedHashMap<String, Object>>) wrapper.getItems();
        int x = 2;
        mainActivity.runOnUiThread(new Runnable() {
            public void run() {
                List<AddressDetail> addressDetailsList = new ArrayList<>();
                for (int i = 0; i < addressDetailsItems.size(); i++) {

                    LinkedHashMap<String, Object> addressDetailsMap = (LinkedHashMap<String, Object>) addressDetailsItems.get(i);
                    ...
                    mainActivity.setArrayAdapter(addressDetailsList);
                }
